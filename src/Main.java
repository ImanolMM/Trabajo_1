...
//hola
    ...
    public static void cargarPalabras (){
        ...
        int numWebs = webs.obtenerNumWebs();
        int numPalabras = palabras.obtenerNumPalabras();

        for (int pos = 0; pos < numWebs; pos ++){
            Web web = webs.devolverWebPorPos(pos);
            String nomWeb = web.obtenerNombre();
            System.out.println("asdasdasd:_  " + pos);

            for (int ind = 0; ind < numPalabras; ind++){
                PClave palabra = palabras.obtenerPalabraPorPos(ind);

                if (nomWeb.contains(palabra.obtenerNombrePalabra())){
